...
- By default, list **recent open PRs** (created within the last 30 days) via `gh pr list --repo strapi/documentation --state open --search "created:>=$(date -v-30d +%Y-%m-%d 2>/dev/null || date -d '30 days ago' +%Y-%m-%d)" --limit 100 --json number,title,author,createdAt`.
- If `--include-old` is set, list all open PRs (no recency filter): `gh pr list --repo strapi/documentation --state open --limit 100 --json number,title,author,createdAt`.
