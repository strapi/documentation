...
argument-hint: "[--non-interactive] [--max-review-fix-rounds <N>] <subject: keywords | Strapi PR (url/number) | Notion URL | Linear issue | PDF path | pasted text>"
user-invocable: true
---
...
- `--max-review-fix-rounds <N>` (canonical), alias `--fix-rounds <N>` (both equivalent) → set `MAX_FIX_ROUNDS=<N>`. Caps how many review→fix→re-review iterations the review phase (Step 4) runs; this skill passes the value through to `/inki:review`. Defaults to `3` if not given.
...
The fix loop itself lives in `/inki:review` (the review→fix→re-review iteration and its `--max-review-fix-rounds` cap are review's own behavior). `/inki:document` does NOT re-implement it; it delegates by invoking `/inki:review` with the right flags and passing the round cap through.

`MAX_FIX_ROUNDS` here defaults to `3` (override with `--max-review-fix-rounds <N>`, alias `--fix-rounds <N>`). `/inki:document` passes this value to `/inki:review` as its cap.

**Behaviour depends on `AUTO`:**

- **`AUTO=true` (`--non-interactive`):** invoke

  ```
  /inki:review --non-interactive --fix --max-review-fix-rounds <MAX_FIX_ROUNDS> <draft>
  ```

  `/inki:review` runs the loop (apply every actionable fix → re-review → repeat up to the cap) without prompting. `--non-interactive` is what suppresses review's per-round confirmation: that is safe here because the draft was generated within this same run, not human-authored.

- **`AUTO=false` (default):** first invoke a single read-only `/inki:review <draft>` (no `--fix`) and show the report. Then ask whether to (a) run the fix loop, (b) proceed to submit as-is, or (c) stop. On (a), invoke `/inki:review --fix --max-review-fix-rounds <MAX_FIX_ROUNDS> <draft>` (without `--non-interactive`, so review shows each round's diff and confirms before writing).

Read the `ROUNDS_RUN` and final open-findings count back from `/inki:review`'s report for the summary. If the cap was reached with findings still open, do NOT loop further here: `/inki:review` already stopped at the cap and reported the remainder. Carry those into the summary and (in interactive mode) tell the user; they decide whether to submit anyway or iterate manually.
...
- This skill never duplicates phase logic. It only resolves the subject, sequences phases, gates, and summarizes. The review fix loop is **not** implemented here: it belongs to `/inki:review` (Step 4 just invokes review with `--fix` and the round cap, and reads back the result). Each phase is run by **invoking its top-level skill** (`/inki:research`, `/inki:write`, `/inki:review`, `/inki:submit`), never by re-implementing that phase's steps inline. In particular, the submit phase MUST go through `/inki:submit` (which itself invokes `/inki:branch` / `/inki:commit` / `/inki:push` / `/inki:pr`); do NOT branch, commit, push, or open the PR with raw `git` / `gh` commands here, as that drops behavior the sub-skills own (e.g. the `/inki:pr` Vercel preview link). Read-only git/gh inspection is fine; mutating git/gh by hand in place of a phase skill is a defect.
...
- `--non-interactive` approves the review fix loop too: it makes `/inki:document` pass `--non-interactive` to `/inki:review`, so review applies fixes without confirming each round. This is safe here only because the draft was generated within this same run. A bare `/inki:review --fix` on existing human-authored content still shows each round's diff and confirms first (review's human-content guard); `--fix` is always required for review to write anything at all.
- The fix loop runs at most `MAX_FIX_ROUNDS` times (default 3 here, `--max-review-fix-rounds <N>` / `--fix-rounds <N>` to change). The loop itself is enforced by `/inki:review`, which never loops unbounded; remaining findings are reported, not forced.
...
```
/inki:document --non-interactive --max-review-fix-rounds 5 "Document Service API filters"
```
