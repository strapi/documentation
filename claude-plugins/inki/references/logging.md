...
Logging is **on by default**. It never writes inside the repository being worked on.
...
- `--verbose-log` → also write each reviewer agent's raw report, not just the consolidated one.
...
│   └── agents/              # only when --verbose-log: one file per agent per round
...
A standalone skill (run outside `/inki:document`) writes only its own subdirectory under the run directory.
