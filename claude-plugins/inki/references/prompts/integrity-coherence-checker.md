...
- **docs_access**: Ability to fetch other documentation pages. Methods, in order of preference:
  1. Local filesystem access (fastest). The docs location varies by setup. Common path: a local clone of `strapi/documentation`. The agent should receive the path as input or detect it from context, never assume a hardcoded location.
