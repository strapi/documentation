...
- **Strapi-specific (badge placement):**
  - **On headings (h1-h6):**
    ...
    - All other badges (`<GrowthBadge />`, `<EnterpriseBadge />`, `<AlphaBadge />`, `<BetaBadge />`, `<FeatureFlagBadge />`, `<CloudProBadge />`, `<CloudStarterBadge />`, `<CloudBusinessBadge />`, `<VersionBadge />`, etc.): must be on a **separate line** after the heading
