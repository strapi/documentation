...
- `COMMANDS.md`: a generated per-command reference (every skill, its description, and its argument signature), produced from each skill's frontmatter by `scripts/gen-commands.sh` (run with `--check` in CI to catch staleness). Linked from the README.
