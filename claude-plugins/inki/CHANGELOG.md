...
- `--yes` / `-y` non-interactive auto mode on `/inki:pr-fix`, `/inki:review`, `/inki:submit`, and `/inki:write`. Skips confirmation prompts so the skills can be chained or scripted.
...
- Safety bracket on `--yes` mode: when `--yes` is used without explicit PR IDs, the skill presents a batch-review prompt (`y` / `n` / `s <PR#>` to skip individual items) before applying any edit.
