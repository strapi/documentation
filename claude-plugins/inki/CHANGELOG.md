...
- Run logging: every run writes a Markdown report tree to `~/.inki/logs/<YYYY-MM-DD-slug>/` with one subdirectory per phase (research/write/review/submit). Overridable with `--log-dir <path>` or the `INKI_LOG_DIR` env var; never written inside the worked-on repo. `--no-log` disables it; `--verbose-log` also saves each reviewer agent's raw report. Centralized in `references/logging.md`.
