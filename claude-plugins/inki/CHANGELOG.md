...
- `/inki:document [--auto] <subject>` skill: end-to-end orchestrator chaining all four families (research → write → review → submit) for a single subject. Gated between each phase by default; `--auto` chains without pauses. Stops if research finds the subject is already documented (no duplicate pages). The `<subject>` is flexible: keywords, a Notion page URL, a Linear issue, a PDF (path or URL), a local file, or pasted text.
...
- `--auto` non-interactive auto mode (with `--yes` / `-y` kept as aliases) on `/inki:document`, `/inki:pr-fix`, `/inki:review`, `/inki:submit`, `/inki:write`, and `/inki:outline`. Skips confirmation prompts so the skills can be chained or scripted.
...
- Safety bracket on `--auto` mode: when `--auto` is used without explicit PR IDs, the skill presents a batch-review prompt (`y` / `n` / `s <PR#>` to skip individual items) before applying any edit.
