...
Content-types, previously known as models, in Strapi v4 have been completely overhauled. Various keys and settings have been removed that are no longer required and relation syntax has been completely changed to be more clear and easily understood.

- [Convert models to content-types](#convert-models-to-content-types)
- [Updating content-type relations](#updating-content-type-relations)
- [Updating lifecycle hooks](#updating-lifecycle-hooks)
...
If the content-type has relations migrating them to Strapi v4 should be done manually in the [schema](/developer-docs/latest/development/backend-customization/models.md#model-schema) of the content-types and **is required**.
...
In Strapi v4, [lifecycle hooks](/developer-docs/latest/development/backend-customization/models.md#lifecycle-hooks) are declared in a `lifecycles.js` file found in `./src/api/<apiName>/content-types/<contentTypeName>/` folder. The `lifecycles.js` file is similar in structure but no longer needs lifecycles to be wrapped in a `lifecycles: {}` object and there are various new parameters passed into the hooks (see [lifecycle hooks documentation](/developer-docs/latest/development/backend-customization/models.md#hook-event-object)).
:::

To convert Strapi v3 model lifecycle hooks to v4 lifecycle hooks:

1. Move/rename the `<modelName>.js` in `./src/api/<apiName>/content-types/` to the proper content-type folder you created in [step 3](#convert-models-to-content-types) of the content-type migration while changing it's name to `lifecyles.js`.
...
2. In each `lifecycles.js` file adjust the structure and move each lifecycle outside of the legacy `lifecycles: {}` object such as the examples below:

  ::: details v3 lifecycles file
...
3. Refactor the model lifecycle hooks to use the new input variables:

  All `params` in Strapi v3 have now been placed in an `event` object like `event.params`. Nested inside of this params object you have access to the `data`, `select` (also known as fields), `where` (also known as filters), `orderBy` (also known as sort), `limit`, `offset`, and `populate`.

  Optionally for all after* events you have access to the `event.result` which contains the result response from the database.

  For more information on the available options, keys, and various other information please see the [hook `event` object](/developer-docs/latest/development/backend-customization/models.hmd#hook-event-object) section.

  ::: details v3 example lifecycle
