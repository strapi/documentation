...
You will configure a database for production. With the setup below, you will only need to set **one environment variable** for the `DATABASE_URL` to connect to your PostgreSQL database. First add the package: https://www.npmjs.com/package/pg-connection-string with `yarn add pg-connection-string` then add the following to `config/env/production/database.js`:
