...
For each locale, the table indicates its default ISO code of the locale, its optional display name and indicates if the locale is set as the default one. From the table, administrators can also:
...
Administrators can add and manage as many locales as they want, based on a pre-created list of locales.
...
2. In the locale addition window, choose your new locale among the *Locales* drop-down list. The latter lists alphabetically all available locales, displayed as their ISO code.
3. (optional) In the *Locale display name* textbox, write new display name for your new locale.
