...
# WHY IT EXISTS
#
# The preflight is a detector, and a detector nobody tested is a detector you
# find out about during the incident. On 2026-09-11 PAT_TOKEN_PIWI expired and
# five workflows reacted badly: docs-self-healing died on an opaque git error,
# automerge-cron reported SUCCESS while `gh pr list` returned 401 and it merged
# nothing. The preflight exists so that never repeats, so it has to be provable
# without waiting for the next expiry.
#
...
# macOS lacks, so the expiry arithmetic is exercised on both platforms.
...
echo
echo "The token is gone (this is the 2026-09-11 case):"
