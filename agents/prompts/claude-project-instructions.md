...
```
Step 1: RUN Router
  ├─ Read router.md spec first (per Execution Protocol)
  ├─ Produce Routing Report artifact
...
Step 2: DISPATCH targets (process in priority order: primary → required → optional)
  │
  ├─ FOR EACH primary target:
  │   │
  │   ├─ IF action is create_page OR action is add_section (substantial):
  │   │   ├─ Read outline-generator.md spec
  │   │   ├─ RUN Outline Generator → produce Outline Report artifact
  │   │   ├─ Read drafter.md spec
  │   │   └─ RUN Drafter in Compose mode → produce Draft artifact
  │   │
  │   └─ IF action is update_section (existing page being modified):
  │       ├─ Read drafter.md spec
  │       └─ RUN Drafter in Patch mode → produce Patch artifact
  │
  ├─ FOR EACH required target:
  │   ├─ Read drafter.md spec (if not already loaded)
  │   └─ RUN Drafter in Patch mode → produce Patch artifact
  │
...
1. **The Execution Protocol still applies.** Read each prompt spec file before executing it — auto-chain does not bypass Step 1 of the Execution Protocol.
...
|--------|------|--------|---------|
| **Orchestrator** | `orchestrator.md` | ✅ Available | Coordinates prompts in sequence for Review and Create/Update workflows |
...
- **In auto-chain mode**, the Orchestrator reads the targets and dispatches automatically — the "Next step" text still serves as documentation but the chain proceeds without manual intervention.
