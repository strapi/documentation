...
**Auto-chain mode (Orchestrator is running the pipeline):** The Orchestrator reads the YAML `targets` array and dispatches automatically — no manual copy-paste needed. The "Next step" text still serves as human-readable documentation of what will happen next. Write it as if addressing the user, but the Orchestrator will also use the target actions to decide routing.
