...
Projects generated with `create-strapi-app` deny `image/svg+xml` by default. The generated `config/plugins.*` file includes `image/*` in `allowedTypes` and `image/svg+xml` in `deniedTypes`:
  ...
  upload: {
    config: {
      security: {
        ...
        deniedTypes: ['image/svg+xml', 'application/x-sh', 'application/x-dosexec'],
      },
    },
  },
};
...
For the full upload security configuration reference, including how to combine `allowedTypes` and `deniedTypes`, see [Media Library: Security](/cms/features/media-library#security).
