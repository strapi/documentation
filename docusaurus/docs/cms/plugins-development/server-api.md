...
<Tldr>
The Server API lets a plugin register lifecycle hooks, expose routes, declare controllers and services, add policies and middlewares, and provide configuration. Everything runs server-side, in contrast to the Admin Panel API which targets the React front end.
</Tldr>
...
The server part of a Strapi plugin is loaded by the Strapi runtime at startup. It is defined in an entry file — typically `server/src/index.js|ts` — that exports an object (or a function returning an object) describing what the plugin contributes to the server: lifecycle hooks, routes, controllers, services, policies, middlewares, content-types, and configuration.

To use the Server API, you tap into this entry file and choose which capabilities to implement.
...
module.exports = {
  'my-content-type': { schema: myContentType },
};
```
