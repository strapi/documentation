...
The `.tar` file generated by the `strapi export` command stores data in a flat, [JSON Lines](https://jsonlines.org/) structure that can be parsed or converted easily:
...
Each folder (except for `metadata.json`) contains one or more `.jsonl` files named sequentially (e.g., `entities_00001.jsonl`). Each line in these `.jsonl` files represents a single record, making the archive convenient for line‑by‑line processing or conversion to formats such as CSV.

To generate an archive that is easy to inspect or edit, disable compression and encryption:
