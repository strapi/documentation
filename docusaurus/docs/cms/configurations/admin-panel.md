...
:::tip
Admin tokens — the new kind of API token for programmatic access to the Admin API — are configured entirely from the admin panel. No additional code-based configuration is required beyond the parameters above. See [Admin tokens](/cms/features/api-tokens#admin-tokens) for the full documentation.
:::
