...
sidebar_position: 1
description: Learn to use the Media Library which allows you to display and manage all assets uploaded in the application.
...
<!-- <Guideflow lightId="mk6z26zaqp" darkId="9r2m74otok"/> temporarily hidden while we're updating it-->

<ThemedImage
  alt="Media Library settings"
  sources={{
    light: '/img/assets/media-library/temp-media-lib-overview-placeholder.png',
    dark: '/img/assets/media-library/temp-media-lib-overview-placeholder_DARK.png',
  }}
/>
...
:::caution
The <Icon name="gear-six" /> button and the view configuration page described below are not available in the <Icon name="images" /> Media Library, which replaces both settings:
...
Both settings still apply to the media field of the <Icon name="feather" /> Content Manager. To change them, set the `useLegacyMediaLibrary` property to `true` temporarily.
:::
...
| `breakpoints`             | Allows you to override the breakpoints sizes at which responsive images are generated when the "Responsive friendly upload" option is set to `true` (see [responsive images](#responsive-images)) | Object | `{ large: 1000, medium: 750, small: 500 }` |
...
:::note
`concurrentUploadRequests` is only read by the Media Library. `concurrentUploadSize` applies to every upload, because the server enforces it.
:::
...
AI clients connected to the [Strapi MCP server](/cms/features/strapi-mcp-server) can browse and manage the Media Library through dedicated tools: 3 read-only tools that list assets, return a single asset and return the folder tree, and 7 write tools that update asset metadata, move and delete assets, and create, rename, move and delete folders (see [Media Library tools](/cms/features/strapi-mcp-server#media-library-tools)).
