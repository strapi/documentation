...
:::tip New projects
Apps scaffolded with `create-strapi-app` include a pre-configured `security` block in the generated `config/plugins.*` file, with `allowedTypes` covering common image, video, audio, and document formats, and `deniedTypes` blocking SVG files as well as executable and shell script types.

Since Strapi <VersionBadge version="5.52.2+" noTooltip />, `image/svg+xml` is part of the generated `deniedTypes`, so SVG uploads are refused even though they match the `image/*` wildcard in `allowedTypes`. SVG files can embed browser-active content such as scripts and event handlers, and an explicit `deniedTypes` entry always takes precedence over a wildcard in `allowedTypes`. This affects newly generated projects only: existing projects keep their current configuration unless you add the same entry yourself.
