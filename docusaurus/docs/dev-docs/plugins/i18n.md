...
:::prerequisites
The Internationalization (i18n) feature is installed by default on all Strapi applications running on version 3.6.0 or higher. For lower versions, a migration is needed (see [Update Strapi version](/dev-docs/update-version)), as well as a manual installation of the plugin.
:::
...
<!-- TODO: the link might change once v5 is out, don't forget to update it -->
...
#### For the default locale {#create-default-locale}
...
#### For a specific locale {#create-specific-locale}
...
#### In a collection type {#create-collection-type}
...
#### In a single type {#create-single-type}
...
#### In a collection type {#delete-locale-collection-type}
...
#### In a single type {#delete-locale-single-type}
...
- The `locale` field is added to the [GraphQL](https://graphql.org/) schema.
...
{
  "homepage": {
    "documentId": "j1k2l3m4n5o6q7r8s9tuvwxy",
...
```graphql
mutation {
  updateHomepage(locale: "fr", title: "Bienvenue sur l'annuaire FoodAdvisor !" ) {
    documentId
    title
  }
}
