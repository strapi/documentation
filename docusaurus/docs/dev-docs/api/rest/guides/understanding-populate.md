...
For instance, the [FoodAdvisor](https://github.com/strapi/foodadvisor) example application includes a `blocks` dynamic zone in the `article` content-type, and the dynamic zone includes 3 different components with different fields. Instead of explicitly defining all the field names to populate, you can choose to populate all fields of all components by passing `[populate=*]`.
