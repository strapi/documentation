/* count land names at the far stop and confirm PROJECTS MANAGEMENT letters */
'use strict';
const { chromium } = require('/Users/piwi/.npm/_npx/e41f203b7505f1fb/node_modules/playwright-core');
(async () => {
  const br = await chromium.launch({ headless: true });
  const page = await br.newPage({ viewport: { width: 1440, height: 900 } });
  await page.goto('http://127.0.0.1:8123/index.html?scale=1&sail=full');
  await page.waitForFunction(() => window.__helm && window.__helm.ready, null, { timeout: 40000 });
  await page.keyboard.press('c');
  await page.waitForFunction('chart.ready === true', null, { timeout: 20000 });
  await page.waitForTimeout(700);
  const r = await page.evaluate(() => {
    const lands = [...document.querySelectorAll('#clgeo .cl-land')].map(e => e.textContent);
    return { n: lands.length, lands };
  });
  console.log(JSON.stringify(r, null, 1));
  await br.close();
})().catch(e => { console.error(e); process.exit(1); });
