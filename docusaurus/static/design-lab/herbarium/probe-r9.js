...
(async () => {
  ...
  await page.goto(BASE, { waitUntil: 'domcontentloaded' });
  await ready(page);
  await shootStates(page, 'r9wave-');
  const diffPage = await ctx.newPage();
  await diffPage.goto('about:blank');
  out.A.pixelDiff = {};
  for (const [name] of STATES) {
    const a = fs.readFileSync(QA + 'r9base-' + name + '.png').toString('base64');
    const b = fs.readFileSync(QA + 'r9wave-' + name + '.png').toString('base64');
  ...
  await page.screenshot({ path: LOG + 'r9-01-slip.jpg', type: 'jpeg', quality: 82 });
  ...
  await page.screenshot({ path: LOG + 'r9-02-slip-close.jpg', type: 'jpeg', quality: 88, clip: slipBox });
  ...
  await page.screenshot({ path: LOG + 'r9-03-beat.jpg', type: 'jpeg', quality: 84, clip: beatBox });
  ...
  await ready(page);
  await settle(page, 700);
  await clickSpecimen('bythedeep');
  await page.keyboard.press('y');
  await page.waitForURL('**/bythedeep/**', { timeout: 5000 });
  ...
  await page.screenshot({ path: LOG + 'r9-04-slip-reduced.jpg', type: 'jpeg', quality: 80 });
  await page.keyboard.press('Escape');
  await page.waitForTimeout(150);
  c = await cleanState();
  ...
  const rt0 = Date.now();
  await page.keyboard.press('y');
  await page.waitForURL('**/secretb/**', { timeout: 4000 });
  ...
  await page.mouse.move(0, 0);
  await page.waitForTimeout(300);
  await page.screenshot({ path: LOG + 'r9-05-opening.jpg', type: 'jpeg', quality: 82 });
  ...
  await page.screenshot({ path: LOG + 'r9-06-ribbon.jpg', type: 'jpeg', quality: 90, clip: cardBox });
  ...
  for (const h of ['#~all', '#~bloom', '#~winter', '#~night', '#~eldest', '#~tended',
    ...
    const v = await page.evaluate(hh => {
      ...
      return {
        ...
        last: sc && sc.lastElementChild ? sc.lastElementChild.id === 'appdx' : false
      };
    }, h);
    out.E.views.push(v);
  ...
  out.E.plate = plate;
  const wantHrefs = { pixelcity: '../pixelcity/', cartastrapiana: '../cartastrapiana/', bythedeep: '../bythedeep/', longway: '../longway/', firstlight: '../firstlight/', secreta: '../secreta/', secretb: '../secretb/' };
  ok(plate.length === 7 && plate.every(p => wantHrefs[p.w] === p.href), 'E seven loans, right doors');
  out.E.hints = [];
  ...
  fs.writeFileSync(LOG + 'probe-r9.json', JSON.stringify(out, null, 1));
