...
(function () {
  ...
  var CSS = ''
    ...
    + '.appdx-env .env-flap{position:absolute;left:-1px;right:-1px;top:0;height:74px;background:linear-gradient(#f2ead9,#e9dfc7);border-bottom:1px solid #cfbf9d;'
      + 'clip-path:polygon(0 0,100% 0,50% 100%);transform-origin:top center;transition:transform .85s cubic-bezier(.32,.85,.28,1)}'
    ...
    + '.appdx-env.open{cursor:default;animation:appdxEnvSettle 1.1s .35s ease both}'
    + '@keyframes appdxEnvSettle{to{opacity:.5;transform:translateY(-6px) scale(.985)}}'
    /* THE OPENING, in three movements. The flap lifts on its fold; the sheet is
       DRAWN OUT of the envelope's mouth, clipped so it appears from inside it
       rather than fading in over the top; and then it UNFOLDS flat, hinged at
       its head the way a folded letter opens. The three overlap by a beat each,
       because a letter does not wait politely between them. */
    + '.appdx{perspective:1600px}'
    + '.appdx-body{transform-origin:top center;animation-name:appdxDraw;animation-duration:1.15s;animation-delay:.18s;'
      + 'animation-timing-function:cubic-bezier(.22,.8,.3,1);animation-fill-mode:both}'
    + '@keyframes appdxDraw{'
      + '0%{opacity:0;transform:translateY(-58px) scale(.9);clip-path:inset(0 6% 100% 6%)}'
      + '40%{opacity:1}'
      + '100%{opacity:1;transform:none;clip-path:inset(0 0 0 0)}}'
    + '.appdx-body .appdx-sheet{transform-origin:top center;animation-name:appdxUnfold;animation-duration:1.25s;'
      + 'animation-delay:1.05s;animation-timing-function:cubic-bezier(.2,.75,.25,1);animation-fill-mode:both}'
    + '@keyframes appdxUnfold{'
      + '0%{transform:rotateX(-82deg) scaleY(.92)}'
      + '55%{transform:rotateX(-22deg) scaleY(.99)}'
      + '100%{transform:none}}'
    /* the sheet lies in shadow while it is still folded over */
    + '.appdx-body .appdx-sheet::after{content:"";position:absolute;inset:0;pointer-events:none;z-index:3;'
      + 'background:linear-gradient(#241a0c,rgba(36,26,12,0));animation-name:appdxFoldShade;animation-duration:1.25s;'
      + 'animation-delay:1.05s;animation-timing-function:ease-out;animation-fill-mode:both}'
    + '@keyframes appdxFoldShade{0%{opacity:.5}70%{opacity:.12}100%{opacity:0}}'
    + '@media (prefers-reduced-motion:reduce){'
      + '.appdx-env .env-flap,.appdx-env .env-seal{transition:none}'
      + '.appdx-body,.appdx-body .appdx-sheet,.appdx-body .appdx-sheet::after{animation:none}'
      + '.appdx-body .appdx-sheet::after{display:none}}'
