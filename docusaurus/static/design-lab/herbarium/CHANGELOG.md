# The Herbarium — the Golden Shore crossing
...
This world's own new thing is **HELICHRYSUM VESPERTINUM**, an everlasting cut on
the cliff road below the lamp room at the last hour of light — dry on the stem
before it was ever picked, and so the only sheet in the cabinet that has not lost
its colour. It arrived after the plate was finished, and a herbarium does not
remake a finished plate for a late arrival: it mounts it on a sheet of its own
and files it directly behind. And one of the six loans, the cartoon kelp, has
gone back to its lender; the plate was re-laid for five so that nothing stands
empty.
...
`qa/r19/everlasting.js` — both plates read, the five sheets counted and their boxes measured so the re-laid plate is proven to have no hole, the late sheet's label read, and the loan slip's Escape / N / Tab / Enter-on-NO proven safe before Y crosses.
...
The plate's own battery (`probe-r10.js`) was re-cut to the plate as it now is:
five loans in the grid laid 3+2, the late accession on its own sheet filed
behind as `#appdx2`, and the prose speaking of five. It had been asserting the
six-loan shape and throwing on the retired specimen, so this world had no
